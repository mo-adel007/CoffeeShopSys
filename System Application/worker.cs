    ...
    public partial class worker : Form
    {
        ...
        private void button_Click(object sender, EventArgs e)
        {
            ...
            if (button.BackColor == Color.SkyBlue)
            {
                ...
                try
                {
                    dbConnection();
                    cmd.Connection = _connect;
                    cmd.CommandText = "Delete  from `bill` where pro_name='" + button.Text +  "'; ";
            ...
            else
            {
                ...
                try
                {
                    dbConnection();
                    cmd.Connection = _connect;
                    cmd.CommandText = "Select * from `product` where 	Product_name='" + button.Text + "'; ";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                cmd.CommandText =
                    "INSERT INTO `corner`.`timing` (`Login_Date_time`,`User_Name`,`Logout_date_time`,`User_id`) VALUES ('" +
                    Convert.ToDateTime(label2.Text).ToString("s") + "' , '"+ label3.Text + "' , '" + now + "' , '" + Convert.ToInt16(UNameID.Text) + "') ; ";
        ...
        private void AddSafe()
        {
            try
            {
                ...
                cmd.Connection = _connect;
                cmd.CommandText =
                    "INSERT INTO `corner`.`safe`(`Type`,`price`,`reason`,`proName`,`quantity`,`ReasonOfT`,`PersonTake`,`UserN`,`Dtime`,`Userid`,`Who_personAdd`) VALUES ('" +
                 comboBox1.Text +"' , '" + Convert.ToDecimal(priceTB.Text) + "' , '" + reasonT + "' , '"+ proN +"' , '" + q +"' , '" + typeT +"' , '" + personT +
                 "' , '" + label3.Text+"' , '" + now +"' , '" + Convert.ToInt32(UNameID.Text) + "' , '"  +  who_add  +"') ; ";
                MySqlDataReader reader2 = cmd.ExecuteReader();
                _connect.Close();
                ...
                cmd.Connection = _connect;
                cmd.CommandText =
                    "INSERT INTO `corner`.`close_shift` (`processT`,`price`,`reason`,`proName`,`quantity`,`ReasonOfT`,`PersonTake`,`UserN`,`Dtime`,`Userid`,`ShiftNumber`,`WhoPersonAdd`)VALUES ('" +
                    processType + "' , '" + Convert.ToDecimal(priceTB.Text) + "' , '" + reasonT + "' , '" + proN + "' , '" + q + "' , '" + typeT + "' , '" + personT +
                    "' , '" + label3.Text + "' , '" + now + "' , '" + Convert.ToInt32(UNameID.Text) + "' , '"+ shift_N + "' , '" + who_add + "') ; ";
                ...
                cmd.Connection = _connect;
                cmd.CommandText =
                    "INSERT INTO `corner`.`close_day`(`processT`,`price`,`reason`,`proName`, `quantity`,`ReasonOfT` , `PersonT`,`UserN`,`Dtime`,`Userid`,`ShiftNumber`,`whoPerson_add`)VALUES ('" +
                    processType + "' , '" + Convert.ToDecimal(priceTB.Text) + "' , '" + reasonT + "' , '" + proN + "' , '" + q + "' , '" + typeT + "' , '" + personT +
                    "' , '" + label3.Text + "' , '" + now + "' , '" + Convert.ToInt16(UNameID.Text) + "' , '" + shift_N + "' , '" + who_add + "') ; ";
        ...
        private void CloseShiftTab()
        {
            ...
            try
            {
                dbConnection();
                cmd.Connection = _connect;
                cmd.CommandText = "Select * from `close_shift` WHERE processT = 'buy' and ShiftNumber = '"+ shNumber +"' ;";
                MySqlDataAdapter mySqlDataAdapterBuy = new MySqlDataAdapter { SelectCommand = cmd };
                DataTable dTableBuy = new DataTable();
                mySqlDataAdapterBuy.Fill(dTableBuy);
                _connect.Close();
                ...
                dbConnection();
                cmd.Connection = _connect;
                cmd.CommandText = "Select * from `close_shift` WHERE `processT` = 'sell' and ShiftNumber = '" + shNumber + "' ;";
                MySqlDataAdapter mySqlDataAdaptersell = new MySqlDataAdapter { SelectCommand = cmd };
                DataTable dTableSell = new DataTable();
                mySqlDataAdaptersell.Fill(dTableSell);
                _connect.Close();
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            try
            {
                dbConnection();
                cmd.Connection = _connect;
                cmd.CommandText = "Select *  from `product` where Product_name ='"+ productsCB.Text + "'; ";
                ...
                _connect.Close();
                    ...
                    foreach (DataRow rowdel in dTableProduct.Rows)
                    {
                        ...
                        cmd.Connection = _connect;
                        cmd.CommandText =
                            "UPDATE `corner`.`product` SET `Store` = '" + store + "' where Product_name ='" + productsCB.Text + "'; ";
        ...
        private void CloseShift_Click(object sender, EventArgs e)
        {
            ...
            { 
                ...
                dbConnection();
                cmd.Connection = _connect;
                cmd.CommandText = "INSERT INTO `corner`.`shift_details`(`ShiftNum`,`TotalSell`, `TotalBuy`, `ProfitShift`)  VALUES ('" +
                    shN +"' , '"+TSell +"' , '"+TBuy+"' , '"+profitShift + "');";
                MySqlDataReader reader3 = cmd.ExecuteReader();
                _connect.Close();
                ...
                dbConnection();
                cmd.Connection = _connect;
                cmd.CommandText = "TRUNCATE TABLE `close_shift`; ";
                MySqlDataReader reader = cmd.ExecuteReader();
                _connect.Close();
        ...
        private void CloseDay_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dbConnection();
                cmd.Connection = _connect;
                cmd.CommandText = "INSERT INTO `corner`.`day_details` ( `DayNum`,`TotalSell`,`TotalBuy`, `ProfitDay`) VALUES ('" +
                                  DNumber + "' , '" + TSell + "' , '" + TBuy + "' , '" + profitDay + "');";
                MySqlDataReader reader3 = cmd.ExecuteReader();
                _connect.Close();
